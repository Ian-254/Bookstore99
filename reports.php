...
require 'connect.php';
$sql = "SELECT 
        customers.names AS customer, books.title, books.price, sales.date_sold, user.names AS user
        FROM customers
        JOIN sales ON customers.id = sales.customer_id
        JOIN books ON books.id = sales.book_id
        JOIN user ON user.id = sales.user_id ORDER BY sales.date_sold DESC";
...
if (isset($_GET["start_date"]) and  isset($_GET["end_date"]))
{
    ...
    $sql = "SELECT 
        ...
        JOIN sales ON customers.id = sales.customer_id
        JOIN books ON books.id = sales.book_id
        JOIN user ON user.id = sales.user_id 
